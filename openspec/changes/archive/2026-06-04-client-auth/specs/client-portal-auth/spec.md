...
The client app MUST provide a `/login` page where a `CLIENTE` user authenticates with email and password against `POST /auth/login` (with `audience = "CLIENTE"`).

#### Scenario: Successful client login
- GIVEN a CLIENTE with a set password
...
### Requirement: Role Gate (CLIENTE only)
The client app MUST refuse to establish a session for a non-`CLIENTE` user, even if the credentials are otherwise valid.

#### Scenario: Admin attempts client login
- GIVEN valid ADMIN credentials
- WHEN submitted at the client `/login`
- THEN login is rejected (API returns 401 for the CLIENTE audience)
